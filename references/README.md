...
| [rules-catalog.md](rules-catalog.md) | 清理规则的**分类评级索引**：11 类规则集规模、tier 分布、引导用户选规则的场景话术 |
| [rulesets/](rulesets/) | 各分类规则的**逐条评级明细**（general/cn/dev/design/ai/game/media），由 `tools/generate_rules_catalog.py` 生成 |
...
| [guardrails.md](guardrails.md) | 受保护路径黑名单规格、门禁层级、围栏（Plan 门禁/热文件/扩展名）、advisory 目语义 |
